...

{
    ...
    if(@$_POST != null){
        ...
        if($price >= '550'){
            ...
            if($code == 998){
                ...
                curl_setopt($x, CURLOPT_REFERER, "https://app.laybycafe.com/process");
    ...
    if(@$_GET != null){
        ...
        $product_ref = @$_GET['m_payment_id'];
        $number = str_replace(['+', '-'], '', filter_var($_GET['amount'], FILTER_SANITIZE_NUMBER_INT));
        $price = @substr(round($number), 0 , -2);
    ...
    if(@$_POST != null){
        ...
        if($price == '1000' or $price >= '550'){
           ...
           // $result = $api->get("/merchants/$merchant_id&$merchant_key/getuser/$email/gateway");
            ...
            $period = @$monthsInfo->maximum_period;
//The price lower than 1k 3 months installment
...
include 'includes/payment_logic.php';
            ...
            if($code == 998){
                ...
                curl_setopt($x, CURLOPT_POSTFIELDS, $data);
                curl_setopt($x, CURLOPT_FOLLOWLOCATION, 0);
                curl_setopt($x, CURLOPT_REFERER, "https://demo.laybycafe.com/process");
                curl_setopt($x, CURLOPT_RETURNTRANSFER, 1);
                $data = curl_exec($x);
                curl_close($x);
                echo $data;
