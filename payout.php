...
$verifyData = json_decode($_GET['verify']);



    $data = Array (

        'merchant_id' => @base64_decode($verifyData->ref_id),
        'payment_id' => @base64_decode($verifyData->payment_id),
        'bank_name' => @base64_decode($verifyData->bank_name),
        'account_holder' => @base64_decode($verifyData->account_holder),
        'bank_account' => @base64_decode($verifyData->bank_account),
        'branch_code' => @base64_decode($verifyData->branch_code),
        'amount' => @base64_decode($verifyData->amount),

    );
    $results = $api ->get("/payout",$data);

$message = json_decode($results->response);
...
<head>
    <meta charset="utf-8">
    <title>Layby Cafe| verify payout</title>
