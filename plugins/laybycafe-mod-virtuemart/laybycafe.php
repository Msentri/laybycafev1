 ...
 * laybycafe.php
 * 
 * Copyright (c) 2012 LaybyCafe (Pty) Ltd
 *
 * LICENSE:
 * 
 ...
 * @copyright   2012 LaybyCafe (Pty) Ltd
 * @license     http://www.opensource.org/licenses/lgpl-license.php LGPL
 * @link        http://www.laybycafe.co.za/help/virtuemart
 * @version     1.3.1
 */
...
define('SANDBOX_MERCHANT_ID', '226196');
define('SANDBOX_MERCHANT_KEY', 'tA8fGWjM8vcmJ39w2kpEseJSxZ67fY2zq2');

class plgVMPaymentLaybyCafe extends vmPSPlugin
{
    // Instance of class
    ...
    function __construct(& $subject, $config) 
    {
    	...
    	$varsToPush = array(
            'laybycafe_merchant_key' => array('', 'char'),
            'laybycafe_merchant_id' => array('', 'char'),
            'laybycafe_passphrase' => array('', 'char'),
    	    'laybycafe_verified_only' => array('', 'int'),
    ...
    function _getPayfastDetails($method)
    {
    	if ($method->sandbox)
        {
            $laybycafeDetails = array(
                'merchant_id' => SANDBOX_MERCHANT_ID,
                'merchant_key' => SANDBOX_MERCHANT_KEY, 
                'url' => 'https://demo.laybycafe.com/process'
            );
        }
        else 
        {
            $laybycafeDetails = array(
                'merchant_id' => $method->laybycafe_merchant_id,
                'merchant_key' => $method->laybycafe_merchant_key,
                'url' => 'https://app.laybycafe.com/process'
            );
        }
    
    	return $laybycafeDetails;
    }

    function _getPaymentResponseHtml($laybycafeData, $payment_name)
    {
        $html = "";
    	/*vmdebug('laybycafe response', $laybycafeData);
    
    	$html = '<table>' . "\n";
    	$html .= $this->getHtmlRow('PAYFAST_PAYMENT_NAME', $payment_name);
    	$html .= $this->getHtmlRow('PAYFAST_ORDER_NUMBER', $laybycafeData['invoice']);
    	$html .= $this->getHtmlRow('PAYFAST_AMOUNT', $laybycafeData['mc_gross'] . " " . $laybycafeData['mc_currency']);
    ...
    {
	   return $this->createTableSQL('Payment LaybyCafe Table');
    }
    ...
    function getTableSQLFields() 
    {
    	$SQLfields = array(
    	    ...
    	    'tax_id' => ' smallint(1)',
    	    'laybycafe_response' => ' varchar(255)  ',
    	    'laybycafe_response_payment_date' => ' char(28)'
    	);
    ...
    function plgVmConfirmedOrder($cart, $order) 
    {
    	...
    	$laybycafeDetails = $this->_getPayfastDetails($method);
    	
        if (empty($laybycafeDetails['merchant_id']))
        {
    	    vmInfo(JText::_('VMPAYMENT_LAYBYCAFE_MERCHANT_ID_NOT_SET'));
    	    return false;
    	}
    	...
    	$post_variables = Array(
            // Merchant details
            'merchant_id' => $laybycafeDetails['merchant_id'],
            'merchant_key' => $laybycafeDetails['merchant_key'],
			...
			// Item details
				"item_name" => vmText::_('VMPAYMENT_TCO_ORDER_NUMBER') . ': ' . $order['details']['BT']->order_number,
        ...
        $passPhrase = $method->laybycafe_passphrase;
        if( empty( $passPhrase ) || ($method->sandbox) )
        {
    	...
    	$dbValues['laybycafe_custom'] = $return_context;
    	...
    	$html = '<form action="' . $laybycafeDetails['url'] .'" method="post" name="vm_laybycafe_form" >';
    	$html.= '<input type="image" name="submit" src="\images\stories\virtuemart\payment\laybycafe.png" alt="Click to pay with LaybyCafe - it is fast, free and secure!" />';
    	...
    	$html.= ' document.vm_laybycafe_form.submit();';
    	$html.= ' </script>';
    ...
    function plgVmOnPaymentNotification() 
    {
        ...
        // Include LaybyCafe Common File
        require_once("laybycafe_common.inc");
        ...
        if( !$pfError && !$pfDone )
        {
            ...
            $pfData = pfGetData();
            $laybycafe_data = $pfData;
        
            pflog( 'LaybyCafe Data: '. print_r( $pfData, true ) );
    	...
    	$order_number = $laybycafe_data['custom_str1'];
    	$virtuemart_order_id = VirtueMartModelOrders::getOrderIdByOrderNumber($laybycafe_data['custom_str1']);
    	...
    	if (!$virtuemart_order_id) 
        {
    	    ...
    	    //$this->sendEmailToVendorAndAdmins(JText::_('VMPAYMENT_LAYBYCAFE_ERROR_EMAIL_SUBJECT'), JText::_('VMPAYMENT_LAYBYCAFE_UNKNOWN_ORDER_ID'));
    	    exit;
    	}
        ...
        $pfHost = ($method->sandbox ? 'sandbox' : 'www') . '.laybycafe.co.za';
    	...
    	$this->logInfo('laybycafe_data ' . implode('   ', $laybycafe_data), 'message');

        
        pflog( 'LaybyCafe ITN call received' );
        ...
        $response_fields['laybycafe_response'] = $pfData['payment_status'];
        $response_fields['laybycafe_response_payment_date'] = date('Y-m-d H:i:s');
    	...
    	if ($virtuemart_order_id && $pfData['payment_status'] == 'COMPLETE') 
        {
    	    ...
    	    $order['comments'] = JTExt::sprintf('VMPAYMENT_LAYBYCAFE_PAYMENT_CONFIRMED', $order_number);
    ...
    function plgVmOnShowOrderBEPayment($virtuemart_order_id, $payment_method_id) 
    {
    	...
    	$html .= $this->getHtmlRowBE('laybycafe_PAYMENT_NAME', $paymentTable->payment_name);
    	//$html .= $this->getHtmlRowBE('laybycafe_PAYMENT_TOTAL_CURRENCY', $paymentTable->payment_order_total.' '.$currency_code_3);
    	$code = "laybycafe_response_";
