 ...
 * @author		WooThemes
 */
define( 'WC_GATEWAY_LAYBYCAFE_VERSION', '1.4.3' );
...
class WC_Gateway_LaybyCafe extends WC_Payment_Gateway {
	...
	public function __construct() {
		...
		$this->method_description = sprintf( __( 'LaybyCafe works by sending the user to %1$sLaybyCafe%2$s to enter their payment information.', 'woocommerce-gateway-laybycafe' ), '<a href="https://laybycafe.com/">', '</a>' );
		...
		if ( 'yes' === $this->get_option( 'testmode' ) ) {
            $this->merchant_id      = '226196';
            $this->merchant_key     = 'tA8fGWjM8vcmJ39w2kpEseJSxZ67fY2zq2';
	...
	public function admin_options() {
		...
		?>
			...
			<div class="inline error"><p><strong><?php _e( 'Gateway Disabled', 'woocommerce-gateway-laybycafe' ); ?></strong> <?php /* translators: 1: a href link 2: closing href */ echo sprintf( __( 'Choose South African Rands as your store currency in %1$sPricing Options%2$s to enable the LaybyCafe Gateway.', 'woocommerce-gateway-laybycafe' ), '<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=general' ) ) . '">', '</a>' ); ?></p></div>
			<?php
		}
	}
	...
	public function generate_laybycafe_form( $order_id ) {
        ...
        foreach( WC()->cart->get_cart() as $cart_item_key => $cart_item ){
            ...
            if($Persons == null || empty($Persons)){
                ...
                //Product Data//
                $payment_plan = $cart_item['wccf']['5178']['value'];
		...
		$this->data_to_send = array(
			...
			'billing_address'    =>     json_encode($order ->get_address()),
		...
		if ( function_exists( 'wcs_order_contains_renewal' ) && wcs_order_contains_renewal( $order ) ) {
			...
			// we will create a new subscription in LaybyCafe and link it to the existing ones in WC.
			// The old subscriptions in LaybyCafe will be cancelled once we handle the itn request.
		...
		if ( $laybycafe_error ) {
			...
			if ( $this->send_debug_email ) {
				...
				if ( isset( $data['pf_payment_id'] ) ) {
					$body .= 'LaybyCafe Transaction ID: ' . esc_html( $data['pf_payment_id'] ) . "\n";
				}
				if ( isset( $data['payment_status'] ) ) {
					$body .= 'LaybyCafe Payment Status: ' . esc_html( $data['payment_status'] ) . "\n";
				}
	...
	public function handle_itn_payment_complete( $data, $order, $subscriptions ) {
		...
		if ( $this->order_contains_pre_order( $order_id )
			...
			if ( ! $is_pre_order_fee_paid ) {
				...
				$order->add_order_note( sprintf( __( 'LaybyCafe pre-order fee paid: R %1$s (%2$s)', 'woocommerce-gateway-laybycafe' ), $data['amount_gross'], $data['pf_payment_id'] ) );
				...
				/* translators: 1: gross amount 2: payment id */
				$order->add_order_note( sprintf( __( 'LaybyCafe pre-order product line total paid: R %1$s (%2$s)', 'woocommerce-gateway-laybycafe' ), $data['amount_gross'], $data['pf_payment_id'] ) );
		...
		if ( $this->send_debug_email ) {
			...
			$body =
				...
				. 'LaybyCafe Transaction ID: ' . esc_html( $data['pf_payment_id'] ) . "\n"
	...
	public function process_pre_order_payments( $order ) {
		...
		if ( is_wp_error( $results ) ) {
			/* translators: 1: error code 2: error message */
			$order->update_status( 'failed', sprintf( __( 'LaybyCafe Pre-Order payment transaction failed (%1$s:%2$s)', 'woocommerce-gateway-laybycafe' ), $results->get_error_code() ,$results->get_error_message() ) );
			return;
		}
	...
	public function setup_constants() {
		...
		define( 'PF_MSG_PENDING', __( 'The payment is pending. Please note, you will receive another Instant Transaction Notification when the payment status changes to "Completed", or "Failed"', 'woocommerce-gateway-laybycafe' ) );
